...
class comment {
  public:
    ...
    comment(std::string cmnt) { text = cmnt; }
};
...
class empty_lines {
  public:
    ...
    empty_lines(unsigned int num = 1) { num_lines = num; }
};
...
class config_file {
  ...
  public:
    ...
    class iterator {
      ...
      public:
        ...
        iterator operator+(const int& i) {
            iterator it;
            // add to the data_order i terator
            it.it = this->it + i;
        ...
        // addition
        void operator+=(const int& i) { this->it += i; }

        // subtraction
        void operator-=(const int& i) { this->it -= i; }

        // dereference operator (TODO: Make this work for write operations as
        // well)
        std::pair<std::string, std::string> operator*() {
            ...
            // set the second part to the value, if it isn't a comment or empty
            // line
            if (*it == EMPTY_LINE) {
                p.first = EMPTY_LINE;
                p.second = "";
    ...
    // initialize the file
    config_file(const std::string& filename, const int mode,
                const char separator = ' ')
    ...
    // a function for write mode that saves changes from the buffer (the data
    ...
    bool write_changes() {
        ...
        if (this->mode != WRITE || this->data.empty()) {
            ...
            // read the order from the vector, and output the values from the
            // buffer
            for (std::string name : this->data_order) {
                if (name.substr(0, COMMENT_LINE.length()) == COMMENT_LINE) {
                    this->file
                        << '#'
                        << name.substr(COMMENT_LINE.length(), std::string::npos)
                        << '\n';
    ...
    void close() {
        ...
        this->data_order.erase(this->data_order.begin(),
                               this->data_order.end());
    }
    ...
    template <typename T>
    std::vector<T> gets(const std::string& name,
    ...
    // a series of functions to write to the data variable, which acts as a
    // buffer before a final write
